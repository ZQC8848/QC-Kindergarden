...
|---|---|---|
| New story | a `stories/<slug>.md` whose slug is not in `state.json` | "📖 新故事" with title, teaser, cast, place, link |
| New illustration | a `stories/assets/<slug>-p<N>-v<M>.png` (or cover `<slug>-v<M>.png`) not in `state.json` | "🖼️ 插图更新" with the count and one attached image |
...
5. **Post.**
   ...
   ```
   The script records what it announced in `state.json`; commit that file with the story so nobody double-posts.
...
- State: `.agents/skills/discord-notify/state.json`, committed.

## Files

- `scripts/notify_discord.py` — detection, message building, posting; Python 3 plus `requests`.
- `state.json` — what has been announced, keyed by story slug and illustration file stem.
