...
Read new exported creation conversations since `evidence_through` in `taste-rules.yaml`. Extract only human-authored or human-labeled events:
...
Periodic automation may perform steps 1–4 and remain quiet when no meaningful candidate exists. It must not perform step 5 without explicit QC approval.
